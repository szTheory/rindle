defmodule Rindle.Storage.GCSConcatenateTest do
  ...
  describe "concatenate/3 (Bypass)" do
    ...
    test "concatenate/3 correctly invokes compose via JSON API", %{bypass: bypass, opts: opts} do
      ...
      assert {:ok, %{key: "final_key", bucket: "test-bucket"}} = GCS.concatenate("final_key", ["part1", "part2"], opts)
    end

    test "concatenate/3 correctly batches sources when count > 32 and folds them", %{bypass: bypass, opts: opts} do
      ...
      assert {:ok, %{key: "final_key", bucket: "test-bucket"}} = GCS.concatenate("final_key", source_keys, opts)
      
      # Assert what happened
      assert_receive {:post, "/storage/v1/b/test-bucket/o/" <> first_tmp_path, first_batch_sources}
      ...
      assert_receive {:post, "/storage/v1/b/test-bucket/o/final_key/compose", second_batch_sources}
    ...
    test "concatenate/3 cleans up source keys after successful composition", %{bypass: bypass, opts: opts} do
      pid = self()
