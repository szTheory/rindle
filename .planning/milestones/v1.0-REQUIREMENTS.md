...
**Archive:** All v1 requirements are checked off below with their final shipped state.
...
- [x] **OPS-01**: `mix rindle.cleanup_orphans` deletes expired upload sessions and their staged storage objects
- [x] **OPS-02**: `mix rindle.cleanup_orphans` accepts `--dry-run` flag that logs affected records without deleting
- [x] **OPS-03**: `mix rindle.regenerate_variants` enqueues Oban jobs for all `stale` or `missing` variants matching given profile/name filters
- [x] **OPS-04**: `mix rindle.regenerate_variants` accepts `--profile` and `--variant` flags for targeted regeneration
- [x] **OPS-05**: `mix rindle.verify_storage` reconciles `media_variants` DB records against actual storage objects and marks missing variants as `missing`
- [x] **OPS-06**: `mix rindle.verify_storage` outputs a summary report: total checked, missing, present, errors
- [x] **OPS-07**: `mix rindle.abort_incomplete_uploads` aborts upload sessions in `signed` or `uploading` state past their TTL to prevent storage cost leaks
- [x] **OPS-08**: `mix rindle.backfill_metadata` re-runs `Rindle.Analyzer` on existing assets and updates stored metadata
- [x] **OPS-09**: All Mix tasks exit with non-zero status code when errors occur, enabling CI/script integration
...
- [x] **CI-01**: CI quality lane runs `mix format --check-formatted` and fails on formatting violations
- [x] **CI-02**: CI quality lane compiles with `--warnings-as-errors` and fails on any compiler warning
- [x] **CI-03**: CI quality lane runs `mix test` with coverage and fails below configured threshold
- [x] **CI-04**: CI quality lane runs Credo and fails on any issue at configured strictness level
- [x] **CI-05**: CI quality lane runs Dialyzer and fails on any type error
- [x] **CI-06**: CI contract lane validates telemetry event names and metadata schemas match documented public contract
- [x] **CI-07**: CI integration lane runs upload, processing, delivery, and cleanup paths against real MinIO + PostgreSQL
- [x] **CI-08**: CI adopter lane runs at least one canonical host integration and verifies end-to-end media lifecycle
- [x] **CI-09**: CI release lane includes dry-run Hex publish and post-publish parity check before any release candidate

### Documentation

- [x] **DOC-01**: Getting started guide is copy-pasteable and demonstrates full upload → variant → delivery flow in Phoenix
- [x] **DOC-02**: Core concepts guide explains asset/variant/session lifecycle with state diagrams
- [x] **DOC-03**: Profile and recipe definitions guide documents all DSL options with working examples
- [x] **DOC-04**: Secure delivery guide explains private-by-default posture, signed URL TTL, and public opt-in
- [x] **DOC-05**: Background processing guide explains Oban worker setup, queue configuration, and retry behavior
- [x] **DOC-06**: Operations and cleanup guide documents all Mix tasks with flags and example output
- [x] **DOC-07**: Failure modes and troubleshooting guide covers quarantine, missing variants, stale detection, and session expiry
- [x] **DOC-08**: All public modules have `@moduledoc` and all public functions have `@doc` with at least one example
