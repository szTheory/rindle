...
| TUS extracted owners | Starting E38-E40 filters name `upload/tus_plug`, but supported annotations emit E38 from `upload/tus_creation` and E39-E40 from `upload/tus_stream` | Preserve both historical filter and emitted finding identities; correct/retain at the emitted owner with `tus_plug_test.exs` creation/concatenation/PATCH proof and `storage/local_tus_test.exs` stream proof. |
...
For each exact `{path, description}` pair: reproduce on supported CI; preserve any distinct starting-filter and emitted-warning identities; identify the owning public/behavior boundary; make the smallest truthful code/spec/pattern correction; run focused behavior tests and SAFE-01; remove exactly the stale filter or disposition the derived finding at its actual path; then inspect the supported Nightly result. Through Plan 126-06, the current slice passes only when its owned warning set is absent or explicitly retained with supported rationale, no new or unowned warning appears, and exhaustive paginated annotation aggregation proves the complete emitted set is E38 text at `tus_creation.ex` plus E39-E40 text at `tus_stream.ex`; Dialyzer must honestly conclude `failure` and Nightly Summary must record that failure. Plan 126-07 owns those extracted modules plus Mux and restores the global green requirement, which remains mandatory for Plans 126-08 and 126-09. A finding is retained only when it is version/dependency analyzer noise or fixing it would require an unsupported contract/schema/telemetry/error change. Record the exact rationale next to the entry. Never suppress a new warning merely to make the gate green.

Every GitHub receipt uses deterministic capture: reuse only a completed exact-head/event run whose named jobs are authoritative; otherwise timestamp immediately before dispatch, capture exactly one post-timestamp branch+event+headSha run, and poll that database ID to terminal. Intermediate expected-failure receipts must not use a failure-sensitive watch as their wait primitive. Final green receipts require successful overall workflow and named-job conclusions. Annotation inspection is exhaustive across every API page and asserts exact complete-set equality.
