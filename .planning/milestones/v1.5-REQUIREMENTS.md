...
direction memos plus project context current at milestone close.
...
- [x] **REPAIR-01**: Operator can re-probe an asset and persist refreshed probe
  fields without mutating unrelated lifecycle state.
- [x] **REPAIR-02**: Operator can requeue failed or cancelled variants for a
  specific asset through an idempotent public repair surface.
- [x] **REPAIR-03**: Operator can regenerate a variant set after preset or
  profile changes through an auditable, explicit operation.
- [x] **REPAIR-04**: Operator can sweep orphaned temp files, stale lifecycle
  rows, and other repairable residue on demand as well as through scheduled
  maintenance.
- [x] **REPAIR-05**: Repair operations emit tagged, operator-readable failure
  reasons and do not silently hide partial failure.

### Runtime Diagnostics & Operational Visibility

- [x] **DIAG-01**: `mix rindle.doctor` detects runtime capability drift,
  missing queues, delivery plug misconfiguration, and stale migration state
  with actionable fix guidance.
- [x] **DIAG-02**: Rindle exposes a documented runtime status report or
  equivalent operator query path for stuck or failed assets, variants, and
  upload sessions.
- [x] **DIAG-03**: Telemetry for repair flows, runtime refusals, and operational
  drift is frozen with documented measurements and metadata.

### Upgrade & Migration Safety

- [x] **UPGRADE-01**: Maintainer can upgrade a pre-v1.4 adopter app into the
  current AV-aware schema/runtime shape using additive migrations and documented
  steps only.
- [x] **UPGRADE-02**: Interrupted AV processing and partial-upgrade states can
  be recovered through documented repair commands that are proven in CI.
- [x] **UPGRADE-03**: Release and upgrade guides teach both greenfield install
...
| Requirement | Outcome |
|-------------|---------|
| PROOF-01..04 | Validated in shipped Phase 29 |
| REPAIR-01..05 | Validated in shipped Phase 30 |
| DIAG-01..03 | Validated in shipped Phase 31 |
| UPGRADE-01..03 | Validated in shipped Phase 32 |
...
| Generic cleanup/refactor bucket | The milestone stayed outcome-based: proof, repair, diagnostics, and upgrade safety only |
...
| REPAIR-01 | Phase 30 | Complete (30-01) |
| REPAIR-02 | Phase 30 | Complete (30-02) |
| REPAIR-03 | Phase 30 | Complete (30-03) |
| REPAIR-04 | Phase 30 | Complete (30-03) |
| REPAIR-05 | Phase 30 | Complete (30-04) |
| DIAG-01 | Phase 31 | Complete (31-01) |
| DIAG-02 | Phase 31 | Complete (31-02) |
| DIAG-03 | Phase 31 | Complete (31-03) |
| UPGRADE-01 | Phase 32 | Complete (32-01) |
| UPGRADE-02 | Phase 32 | Complete (32-02) |
| UPGRADE-03 | Phase 32 | Complete (32-03) |

**Coverage:**
- v1.5 requirements: 15 total
- Completed at close: 15
- Unmapped: 0

---
*Archived at milestone close: 2026-05-06*
