# Milestone v1.7: GCS Resumable Adapter
...
Milestone `v1.7` productized `Rindle.Storage.GCS` as a real second storage
adapter, promoted GCS-style resumable uploads from reserved capability to
shipped contract, and extended the existing adopter-owned runtime posture to
cover resumable session lifecycle, maintenance, observability, onboarding, and
package-consumer proof without turning Rindle into a file server.
...
| 39 | Resumable Adapter Behaviour + Broker Wiring | RESUMABLE-04..08 (5) | 4 | ~4 days | MEDIUM |
...
**Totals:** 5 phases, 17 plans, 18 requirements covered.
...
Plans:
- [x] 38-01-PLAN.md — Packaged resumable migration template + `doctor.resumable_session_schema` drift check (Wave 1, RESUMABLE-01)
- [x] 38-02-PLAN.md — `MediaUploadSession` resumable fields + `UploadSessionFSM` `"resuming"` lane + `Inspect` redaction (Wave 1, RESUMABLE-02)
- [x] 38-03-PLAN.md — Resumable telemetry helper + public contract freeze + redaction parity coverage (Wave 2, RESUMABLE-03)
...
4. End-to-end resumable upload against a real GCS bucket: a test fixture
   streams chunked PUTs via Finch from a streamed body to a session URI
   ...
   silent fallback.
5. Every locked public resumable error shape from `v1.6-CANDIDATE-GCS.md` §4 is
   ...
   `:missing_bucket`, `:storage_object_missing`. Region pinning remains
   advisory metadata/telemetry, not a returned initiation error.
...
Plans:
- [x] 39-01-PLAN.md — Storage resumable behaviour/typespec contract + capability semantics + cross-adapter honesty tests (Wave 1, RESUMABLE-04, RESUMABLE-07)
- [x] 39-02-PLAN.md — GCS resumable client helpers + adapter callback implementation + protocol/error mapping coverage (Wave 2, RESUMABLE-05, RESUMABLE-08)
- [x] 39-03-PLAN.md — Broker resumable initiate/status/cancel lifecycle + compensation + facade parity delegates (Wave 3, RESUMABLE-06, RESUMABLE-07)
- [x] 39-04-PLAN.md — Secret-gated live GCS resumable proof + contract/error closure coverage (Wave 4, RESUMABLE-08)
...
**Plans:** 3 plans. Plan-count guidance:
...
Plans:
- [ ] 41-01-PLAN.md — Deep GCS resumable guide, adapter-honest capability matrix, and optional quickstart pointers
- [ ] 41-02-PLAN.md — Warning-capable `mix rindle.doctor` plus resumable-aware GCS diagnostics
- [ ] 41-03-PLAN.md — Generated-app GCS consumer proof, live CI lane, and cleanup/log hygiene
...
|-------|----------------|--------|-----------|
| 37 — GCS Adapter Foundation | 4/4 | Complete | 2026-05-07 |
| 38 — Resumable Persistence + FSM | 3/3 | Complete | 2026-05-07 |
| 39 — Resumable Adapter Behaviour + Broker Wiring | 4/4 | Complete | 2026-05-07 |
| 40 — Maintenance + Cancel Contract | 3/3 | Complete | 2026-05-07 |
| 41 — Onboarding + Docs + Doctor + Package-Consumer Proof | 3/3 | Complete | 2026-05-08 |
...
- Shipped a real `Rindle.Storage.GCS` adapter with live-bucket parity for
  `store/3`, `download/3`, `delete/2`, `head/2`, and `url/2`, including V4
  signed URLs, profile-aware doctor checks, and the secret-gated `gcs-soak`
  CI proof lane.
- Added resumable session persistence, the `"resuming"` FSM state, telemetry
  for resumable status and cancel operations, and a hard redaction boundary so
  `session_uri` bearer credentials never leak through `inspect/2`,
  `runtime_status`, or telemetry metadata.
- Promoted resumable support into the public storage and broker contracts with
  four new `Rindle.Storage` optional callbacks, three broker entrypoints, the
  locked resumable error vocabulary, and a real-bucket end-to-end resumable
  upload proof.
- Extended the maintenance lane so resumable cancel and cleanup are idempotent,
  retry-safe, and observable through additive abort/cleanup counters plus
  bounded resumable runtime-status reporting.
- Closed the adopter story with a deep GCS guide, warning-capable
  `mix rindle.doctor` diagnostics, and a generated-app `gcs` package-consumer
  proof plus a separate secret-gated `package-consumer-gcs-live` CI lane.

**Issues Deferred:**

- Browser → Mux direct creator upload (`MUX-20..23`) remains deferred to
  `v1.8+`.
- tus resumable protocol work (`TUS-01..19`) remains the locked `v1.8`
  candidate.
- Advisory Phase 34/35 code-review polish remains available for future cleanup
  but did not block `v1.7` shipment.

**Technical Debt / Notes:**

- Provider-backed live proofs now close via accepted CI-only evidence
  (`ci_verified`) instead of manual human UAT files; stale human-UAT artifacts
  must be reconciled when this model changes.
- IAM SignBlob signing and customer-supplied session-URI flows remain out of
  scope until a future milestone explicitly expands the auth surface.
