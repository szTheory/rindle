...
- [ ] **CANCEL-01**: Adopter can cancel an in-flight direct creator upload via
  `Rindle.Streaming.cancel_direct_upload/1` using the `asset_id` returned from
  `create_direct_upload/2`.
- [ ] **CANCEL-02**: Cancel returns `:ok` when the upload is already terminal
...
- [ ] **CANCEL-03**: `create_direct_upload/2` persists the provider `upload_id`
...
|-------------|-------|--------|
| CANCEL-01 | 64, 65 | pending |
| CANCEL-02 | 64, 65 | pending |
| CANCEL-03 | 64 | pending |
