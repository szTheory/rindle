...
- [ ] **TUS-01**: Adopter mounts `Rindle.Upload.TusPlug` (a bare `@behaviour Plug`
...
- [ ] **TUS-02**: tus client creates a resumable upload — `POST` with
...
- [ ] **TUS-04**: `OPTIONS` advertises `Tus-Version`, `Tus-Resumable`,
  ...
  and `Tus-Max-Size`.
- [ ] **TUS-05**: tus upload URLs are HMAC-signed via `Plug.Crypto.sign/verify`
