...
- [ ] **SEC-01**: Magic-byte MIME detection is performed on every upload using file header bytes, never trusting client `Content-Type`
- [ ] **SEC-02**: MIME type is validated against profile allowlist after magic-byte detection; rejection transitions asset to `quarantined`
- [ ] **SEC-03**: File extension is validated against profile allowlist; mismatch between extension and detected MIME is rejected
- [ ] **SEC-04**: File size is validated against profile `max_bytes` limit before processing proceeds
- [ ] **SEC-05**: Pixel count (width × height) is validated against profile limit to prevent decompression bomb variants
- [ ] **SEC-06**: Storage keys are generated by Rindle (UUID-based or hash-based); no user-controlled path component is accepted
- [ ] **SEC-07**: Filenames submitted by clients are sanitized before storage; sanitized name is stored separately from storage key
- [ ] **SEC-08**: Direct upload assets are not promoted until server-side verification confirms object exists and passes validation
