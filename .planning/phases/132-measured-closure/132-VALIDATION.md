...
- **After Plan 132-12:** Run the process-level publication/canonical-population/completed-state regressions and shell syntax check; publication success must become sample 1 with nine later triggers, failed/cancelled/attempt-2 publication must consume sequence 1, extra eligible rows and forged/stale completed PASS evidence must fail closed, and live-valid completed state must pass without mutation.
...
- **After Plan 132-14:** Record a new post-132-13 preserved subject, prove its exact three-executable-path transition from the prior subject, then run the complete ordered preservation census, one authoritative coverage command, and mutation-free no-publish preflight with that new subject before any live collector is authorized.
- **During Plan 132-15:** Keep offline contract feedback before live mutation, then require publication sample 1 plus nine label samples, full eligible-population equality, explicit verify, completed-state revalidation, no eleventh run/state/receipt mutation, and post-sample topology/summary checks as one uninterrupted terminal acceptance chain.
...
| 132-12-01 | 12 | 12 | CI-14, SAFE-02 | T-132-12-01 / T-132-12-02 / T-132-12-03 / T-132-12-04 | Collection and verification share full exact-head eligible-population equality; publication success is sample 1 with nine triggers, nonqualifying publication consumes sequence 1, and forged/stale/extra-population completed PASS fails without mutation | process-level controller regression | `mix test test/install_smoke/ci_timing_automation_test.exs --seed 0 && bash -n scripts/ci/collect_pr_timing_receipt.sh && ./scripts/maintainer/automation_first_contract.sh` | ✅ extend | ⬜ pending |
...
| 132-14-01 | 14 | 13 | CI-14, COV-05, SAFE-02 | T-132-14-01 / T-132-14-02 / T-132-14-03 / T-132-14-04 | A new post-132-13 immutable subject is source-bound to exactly the Plan 12 controller/test, Plan 13 formatter-only test, and planning transition, then passes formatter/topology/quality/SAFE-01/coverage/packed-consumer preservation, inclusive 82.13% integer proof, and mutation-free no-publish preflight | ordered preservation acceptance | Run the exact multiline `132-14-PLAN.md` automated command: derive prior/correction/new-preserved SHAs, require the new preserved SHA equals HEAD, exact-match the three non-planning paths, formatter-reproduce the lane-split delta, source-check canonical population/live revalidation, then snapshot and equality-check every PR/run/label/state/lock/receipt surface around `preflight --no-publish` | ✅ | ⬜ pending |
| 132-15-01 | 15 | 14 | CI-14, COV-05, SAFE-02 | T-132-15-01 / T-132-15-02 / T-132-15-03 / T-132-15-04 / T-132-15-05 | One locked live collector admits the qualifying publication run as sample 1 and exactly nine label runs; selected IDs equal all ten eligible exact-head successes, and explicit/completed verification causes no eleventh run or receipt/state mutation | terminal live automated acceptance | Run the exact multiline `132-15-PLAN.md` automated command: require remote head is a strict ancestor, execute one publish-head controller, assert state provenance binds one publication run plus nine label-run IDs, exact selected/API population equality and ten total runs, run explicit verify, snapshot receipt/current markers/state/all run IDs/eligible IDs, rerun completed state, and assert every snapshot plus label/lock cleanup remains unchanged | ✅ | ⬜ pending |
...
- [ ] 132-12 extends the existing process-level controller suite with publication-success-plus-nine, publication-failure bounded-sequence, exact-population/no-eleventh, extra-eligible rejection, forged/stale completed-state, and positive no-retrigger resume cases; no new framework or dependency is required.
