...
| 132-03-02 | 03 | 3 | CI-14 | T-132-02 / T-132-03 | Receipt source manifest is one contiguous ten-run block in the full eligible same-SHA API sequence, no omitted same-SHA run interval or eligible predecessor/successor overlaps its first-start-through-final-summary window, no baseline ID overlaps, and recomputed median/p95 both meet their inclusive thresholds | automated live-evidence gate | Plan 03 Task 2 read-only `gh api`/`jq` source-manifest verification | ✅ inline plan gate | ⬜ pending |
