...
- **Intermediate supported finalizers (Plans 126-02 through 126-06):** Require the current and earlier owned warning sets to be absent or explicitly retained with supported rationale; inspect exact Dialyzer annotations and log count so the only permitted emitted set is E38 at `lib/rindle/upload/tus_creation.ex` plus E39-E40 at `lib/rindle/upload/tus_stream.ex` (the supported annotations corresponding to the later-owned Tus ledger rows); require Dialyzer `failure`, Nightly Summary `success`, and its log to record `DIALYZER: failure`. Any different count, path, message, or conclusion fails the plan.
