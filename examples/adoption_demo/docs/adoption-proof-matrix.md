...
| Admin screenshot polish | Host env + MinIO | Live admin light/dark screenshot matrix with asserted ignored PNG output, plus a deterministic visual-polish gate on every capture state (no clipped text, WCAG contrast, 44px target sizes, no interactive overlap, stable raster dimensions) — replaces human screenshot review | `e2e/admin-screenshots.spec.js`, `e2e/support/admin-polish.js`, `examples/adoption_demo/test-results/admin-screenshots/` | Merge-blocking (`adoption-demo-e2e`) |
